...
export function addVitePlugin(
  root: Collection,
  { from, constructor, named, options }: { from: string; constructor: string; named?: boolean; options?: string },
): void {
  ...
  if (plugins.find('call_expression', { function: constructor }).size() > 0) return // already registered
  plugins.append(`${constructor}(${options ?? ''})`)
  root.ensureImport(named ? `import { ${constructor} } from "${from}"` : `import ${constructor} from "${from}"`)
}
...
export function mergeObject(object: Collection, source: ConfigObject): void {
  for (const [key, value] of Object.entries(source)) {
    ...
    else if (typeof value !== 'string' && pair.field('value').type === 'object') mergeObject(pair.field('value'), value)
...
function directPair(object: Collection, key: string): Collection {
  return object.children().filter((pair) => pair.type === 'pair' && unquote(pair.field('key').text) === key).first()
}
...
function serialize(value: ConfigValue): string {
  ...
  return `{ ${Object.entries(value).map(([key, v]) => `${propKey(key)}: ${serialize(v)}`).join(', ')} }`
}
