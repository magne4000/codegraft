...
export interface BatiContext extends Record<string, unknown> {
  ...
  filename?: string
  /** Set by the codemod when it strips a `$$.includeIfImported` directive. */
  includeIfImported?: boolean
  /** Surviving relative import specifiers, for Bati's `include-if-imported` graph. Provide the Set. */
  imports?: Set<string>
}
