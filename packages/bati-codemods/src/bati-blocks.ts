...
export const batiBlocks = defineCodemod<BatiContext>({ namespace: '$$' }, (root, ctx) => {
  ...
  function removeBetween(parent: Collection, start: number, end: number): void {
    ...
    parent.findComments().filter((c) => c.node.parent === parent.node && within(c)).remove()
  }
})
...
function toMarker(comment: Collection): Marker {
  ...
  const condition =
    kind === 'if' || kind === 'elif' ? (comment.text.match(/\$\$\.(?:if|elif)\(([\s\S]*)\)/)?.[1]?.trim() ?? null) : null
  return { kind, condition, comment, start: comment.node.documentStartIndex, end: comment.node.documentEndIndex, parent: comment.node.parent }
}
