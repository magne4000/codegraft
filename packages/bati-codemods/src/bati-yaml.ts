...
export const batiYaml = defineCodemod<BatiContext>({ namespace: '$$' }, (root, ctx) => {
  ...
  const items = [...root.find('block_mapping_pair').map((c) => c), ...root.find('block_sequence_item').map((c) => c)].sort(byStart)
  const comments = root.findComments().map((c) => c).sort(byStart)
  ...
  for (const comment of comments) {
    ...
    if (root.evaluateExpression(condition, ctx)) {
      ...
      commentBlockAbove(target, comments).forEach((c, i) => c.remove({ wholeLines: true, collapseBlankBefore: i === 0 }))
