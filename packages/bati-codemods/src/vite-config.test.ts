...
import { addVitePlugin, mergeObject, defineConfigArg } from './vite-config.js'
...
describe('addVitePlugin', () => {
  ...
  it('supports a named import and constructor options', async () => {
    const t = await build((root) =>
      addVitePlugin(root, { from: '@sentry/vite-plugin', constructor: 'sentryVitePlugin', named: true, options: '{ org: "o" }' }),
    )
    const out = t.transform(config('  plugins: [vike()],'), {})
