...
// Formatting is an apply-time choice now — `transform(src, ctx, { format: true })` — not part of the
// codemod. The codemod body is format-agnostic; these helpers build a tsx transformer for it and
// apply it with formatting on (`onFormat`) or off (`verbatim`, the byte-identical default).
const onFormat = async (fn: (root: Collection<'tsx'>) => void, src: string, ctx: Record<string, unknown> = {}): Promise<string> => {
  const t = await defineCodemod(fn).forTarget('tsx')
  return t.transform(src, ctx, { format: true })
}
const verbatim = async (fn: (root: Collection<'tsx'>) => void, src: string, ctx: Record<string, unknown> = {}): Promise<string> => {
  const t = await defineCodemod(fn).forTarget('tsx')
  return t.transform(src, ctx)
}

// Indentation-aware insertion + line-collapse — the `format` apply-time option. Off by default, so
// the verbatim behaviour stays byte-identical; on, inserted/replaced code adopts the file's indent
// unit and EOL and a removed node's line collapses.
describe('transform — format option', () => {
  it('appends a sibling statement at the block indentation', async () => {
    const out = await onFormat((root) => root.find('statement_block').first().append('c()'), 'function f() {\n  a()\n  b()\n}')
    expect(out).toBe('function f() {\n  a()\n  b()\n  c()\n}')
  })

  it('prepends a sibling statement at the block indentation', async () => {
    const out = await onFormat((root) => root.find('statement_block').first().prepend('a()'), 'function f() {\n  b()\n}')
    expect(out).toBe('function f() {\n  a()\n  b()\n}')
  })

  it('re-indents a multi-line replacement to the anchor', async () => {
    const out = await onFormat(
  ...
  it('fills an empty block, one level in and closing on its own line', async () => {
    const out = await onFormat((root) => root.find('statement_block').first().append('return 1'), 'function f() {}')
    expect(out).toBe('function f() {\n  return 1\n}')
  })
  ...
  it('detects a four-space unit and uses it to fill an empty nested block', async () => {
    const out = await onFormat((root) => root.find('statement_block').at(1).append('return 1'), 'function outer() {\n    if (x) {}\n}')
  ...
  it('uses the detected EOL for ensureImport', async () => {
    const out = await onFormat((root) => root.ensureImport("import b from 'b'"), 'import a from "a"\r\nx()')
    expect(out).toBe('import a from "a"\r\nimport b from \'b\'\r\nx()')
  })

  it('leaves comma-separated containers inline even with format on', async () => {
    expect(await onFormat((root) => root.find('array').first().append('3'), 'const a = [1, 2]')).toBe('const a = [1, 2, 3]')
  })
  ...
  it('appends to a multi-line array without a trailing comma, adding only the separating comma', async () => {
    ...
    const out = await onFormat((root) => root.find('array').first().append('3'), 'const a = [\n  1,\n  2\n];')
  ...
  it('appends an interface member with a `;` separator, not a comma, keeping the layout', async () => {
    ...
    const out = await onFormat((root) => root.find('interface_body').first().append('b: B'), 'interface C {\n  a: A;\n}')
  ...
  it('appends an `object_type` member with a `;` separator', async () => {
    const out = await onFormat((root) => root.find('object_type').first().append('c: C'), 'type T = {\n  a: A;\n  b: B;\n};')
  ...
  it('keeps an inline interface body on one line, `;`-separated (not comma-joined)', async () => {
    ...
    const out = await onFormat((root) => root.find('interface_body').first().append('b: B'), 'interface C { a: A; }')
    expect(out).toBe('interface C { a: A; b: B; }')
  })

  it('prepends to a multi-line object on its own line (mirror of append)', async () => {
    const out = await onFormat((root) => root.find('object').first().prepend('x: 0'), 'const a = {\n  a: 1,\n  b: 2,\n};')
    expect(out).toBe('const a = {\n  x: 0,\n  a: 1,\n  b: 2,\n};')
  })
  ...
  it('prepends an interface member with a `;` separator, matching the body style', async () => {
    ...
    expect(await onFormat(prepZ, 'interface C {\n  a: A;\n}')).toBe('interface C {\n  z: Z;\n  a: A;\n}')
  ...
  it('prepends inline before the first element, keeping brace padding (`;`, not `,`)', async () => {
    expect(await onFormat((root) => root.find('array').first().prepend('0'), 'const a = [1, 2]')).toBe('const a = [0, 1, 2]')
    expect(await onFormat((root) => root.find('object').first().prepend('x: 0'), 'const a = { a: 1 }')).toBe(
      'const a = { x: 0, a: 1 }',
    )
    expect(await onFormat((root) => root.find('interface_body').first().prepend('z: Z'), 'interface C { a: A; }')).toBe(
      'interface C { z: Z; a: A; }',
    )
  })

  it('pads an empty brace container when filling its first element under format', async () => {
    expect(await onFormat((root) => root.find('object').first().append('a: 1'), 'const a = {}')).toBe('const a = { a: 1 }')
    expect(await onFormat((root) => root.find('interface_body').first().append('a: A'), 'interface C {}')).toBe(
      'interface C { a: A }',
    )
    // arrays / arg-lists are not padded
    expect(await onFormat((root) => root.find('array').first().append('3'), 'const a = []')).toBe('const a = [3]')
  })

  it('without format, prepend and empty-fill stay byte-identical (verbatim contract)', async () => {
    expect(await verbatim((root) => root.find('array').first().prepend('0'), 'const a = [1, 2]')).toBe('const a = [0, 1, 2]')
    expect(await verbatim((root) => root.find('object').first().append('a: 1'), 'const a = {}')).toBe('const a = {a: 1}')
  })
  ...
  it('collapses the line of an emptied own-line call argument under format', async () => {
    const out = await onFormat((root) => root.find('arguments').first().children().at(0).remove({ separator: true }), 'cfg(\n  arg\n);')
    expect(out).toBe('cfg(\n);')
  })
  ...
  it('collapses nested conditionals in one pass under format (unwrap then remove inside)', async () => {
    ...
    expect(t.transform('if ($$.a) {\n  if ($$.b) {\n    yes();\n  }\n  no();\n}', { a: true, b: false }, { format: true })).toBe(
      '  no();',
    )
  })
  ...
  it('without the format option, an appended statement still lands at column 0', async () => {
    const out = await verbatim((root) => root.find('statement_block').first().append('c()'), 'function f() {\n  a()\n  b()\n}')
    expect(out).toBe('function f() {\n  a()\n  b()\nc()\n}')
  })
})
